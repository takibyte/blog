...
In the previous post, I showed how I made the fundamentals of the port scanner: setting up an initial socket connection within a function, and then looping that function through a range of ports.
...
And there we go, in this post we have: implemented error handling to carefully handle some errors, implemented multi-threading whilst protecting from race conditions with `threading.Lock()`.

In the next and final post, I will be adding argparse functionality — so the port scanner is CLI interactive — as well as add some other polishing touches.
